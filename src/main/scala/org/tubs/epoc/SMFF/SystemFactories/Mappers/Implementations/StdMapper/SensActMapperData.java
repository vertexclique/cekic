...
public class SensActMapperData extends AbstractMapperData{
  // parameters for this mapper
  private float kPredecessor = 3.0f;      // factor that is applied to mass if predecessor is on that resource
  private float kSuccessor = 3.0f;        // factor that is applied to mass if successor is on that resource
  private float kSameApp = 0.0000000001f; // factor that is applied to mass if resource has part of the same application which is not predecessor or successor
  private float kResDist = 1.5f;          // factor that is applied to mass per distance between two resources
   ...
   */
  public SensActMapperData(float predecessor, float successor,
      float sameApp, float resDist, long seed) {
    super(seed);
